    ...
    func beginRefresh() {
        ...
        if config.parkVisible {
            DispatchQueue.main.async {
                UIView.animate(withDuration: self.config.animationDurationFast, animations: {
                    self.parkVisible(true)
                }, completion: { (parkfinished) in
...
public final class YYRefresh: UIView {
	...
	private var refreshingCallback: ((YYRefresh) -> Void)? = nil
    
    // MARK: - Initialization
    public init(position: YYRefresh.Position, config: YYRefresh.Config? = nil, refreshView: YYRefreshView? = nil, action: ((YYRefresh) -> Void)?) {
...
extension YYRefresh {
	...
	override public func layoutSubviews() {
		...
		guard let superV = superview, bounds.width != superV.bounds.width else {
			return
		}
		layoutViews()
	}
...
private extension YYRefresh {
	...
	func layoutViews() {
		...
		if position == .left || position == .right {
			_setAnchorPointFixedFrame(CGPoint.zero)
			transform = self.transform.rotated(by: YYRefresh.degreesToRadians(90))
		}
	}
